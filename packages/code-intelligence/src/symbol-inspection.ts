...
const navigationItems = (
  ...
  !value
    ...
    : Array.isArray(value)
    ? value
    : [value as Location | LocationLink];
...
const key = ({ uri, selectionRange }: Located) =>
  `${uri}\0${rangeText(selectionRange)}`;

const unique = <Item>(items: readonly Item[], itemKey: (item: Item) => string) =>
  items.filter((item, index) =>
    items.findIndex((candidate) => itemKey(candidate) === itemKey(item)) === index
  );
...
const locationGroups = (items: readonly Located[], root: string) =>
  groups(items).flatMap(({ uri, items: related }) => [
    ...
    ...related.map((item) =>
      `  ${locationText(item)}${item.sourceLine ? `  ${item.sourceLine.trim()}` : ""}`
    ),
  ]);
...
const callGroups = (
  ...
  groups(calls.map((call) => ({ ...call, uri: call.item.uri }))).flatMap(
    ({ uri, items }) => [
      workspacePath(uri, root),
...
const symbolAtDefinition = async (
  ...
  return flattenSymbols(symbols, definition.uri).find((symbol) =>
    key(symbol) === key(definition)
  );
};
...
export const inspectSymbol = async (
  ...
  const declaration = definitions.find((definition) =>
    key(definition) === key(base)
  );
  ...
  const extras = (items: readonly Located[]) =>
    items.filter((item) => key(item) !== key(primary));
  const additionalDefinitions = extras(definitions);
  const distinctImplementations = extras(implementations).filter((item) =>
    !definitions.some((definition) => key(definition) === key(item))
  );
  const distinctTypes = extras(typeDefinitions).filter((item) =>
    !definitions.some((definition) => key(definition) === key(item)) &&
  ...
  return {
    ...
    text: [
      ...
      ...(additionalDefinitions.length
        ? ["", `Additional definitions (${additionalDefinitions.length})`, ...locationGroups(additionalDefinitions, root)]
        : []),
      ...(distinctImplementations.length
        ? ["", `Implementations (${distinctImplementations.length})`, ...locationGroups(distinctImplementations, root)]
        : []),
      ...
      ...(shownReferences.length < otherReferences.length
        ...
        : []),
      ...(source
        ? ["", `Source · ${workspacePath(primary.uri, root)}:${rangeText(primary.range)}`, ...source]
        : []),
    ].join("\n"),
  };
};
