...
const inspectOptions = {
  ...observedFileInput,
  includeSource: type("boolean").describe(
    "Include the complete symbol body.",
  ).default(false),
  includeTypeDefinitions: type("boolean").describe(
    "Include callable type-definition targets.",
  ).default(false),
...
export const registerNavigationTools = (
  ...
  server.registerTool(
    ...
    async ({
      ...
      const workspace = await workspaces.get(root);
      const { project, symbols } = await createCodeIntelligence(workspace)
        .workspaceSymbols(
          file,
          query,
          signal,
        );
      const output = symbols === null
        ? null
        : raw
  ...
  server.registerTool(
    ...
    async ({
      ...
      const workspace = await workspaces.get(root);
      const { textDocument, definitions } = await createCodeIntelligence(
        workspace,
      ).definitions(file, position, signal);
      const diagnosticContext = requestDiagnosticContext(
        workspace,
        textDocument,
        root,
