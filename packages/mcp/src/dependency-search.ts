...
const formatResult = (input: {
  ...
  [
    ...
    ...(input.result.content
      ? input.result.content
          .split("\n")
          .map((line, index) => `${input.result.start_line + index}|${line}`)
      : []),
  ].join("\n");
  ...
  async (request: {
      ...
      request.packages.map(async (name): Promise<DependencySearchResult> => {
        try {
          ...
          const searchRoot = subModuleName.includes("/")
            ? path.join(packageRoot, subModuleName.slice(0, subModuleName.indexOf("/")))
            : packageRoot;
    ...
    return [
      ...
      ...results.flatMap((result) => {
        const topScore = "error" in result ? undefined : result.page.results[0]?.score;
        return [
          `## ${
            "error" in result
              ? result.requested
              : `${result.name}@${result.version}${
                  result.name === result.requested ? "" : ` · requested ${result.requested}`
                }`
          }`,
          ...("error" in result
            ? [`Error: ${result.error}`]
            : [
                ...
                ...result.page.results.flatMap((match, index) => [
                  `### ${index + 1} · relevance ${
                    topScore && topScore > 0 ? Math.round((match.score / topScore) * 100) : 0
                  }%`,
                  formatResult({
