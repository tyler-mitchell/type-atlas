...
export const serverInstructions = [
  ...
  "",
  "Start with `inspect_symbol`. It composes definitions, types, implementations, callers, calls, and remaining references into one call, and its reference list shows only what is not already explained as a call or definition. Prefer it over separate `definitions`, `references`, and `callers` calls; reach for those individually when you need one relationship in full.",
  "",
  "Reference scope: `references`, `inspect_symbol`, and `callers` are bounded to the TypeScript project that owns `file`, and each result states that project. In a multi-project workspace, usages from other packages are not included, so treat these as scoped answers rather than a complete audit. Use `workspace_symbols` to find a declaration across loaded projects.",
  "",
  ...
  "",
  "Diagnostics: file-scoped tools already report ambient errors and warnings, so call `diagnostics` only to expand that notice or inspect a file deliberately.",
  "",
