...
const oldHunkLines = (lines: readonly string[]) =>
  lines.filter((line) => line[0] !== "+" && line[0] !== "\\").map((line) => line.slice(1));

const firstSequenceIndex = (lines: readonly string[], sequence: readonly string[]) => {
...
const isPositionFaithful = (source: string, patch: StructuredPatch) => {
  const lines = sourceLines(source);
  return patch.hunks.every(
    (hunk) =>
      firstSequenceIndex(lines, oldHunkLines(hunk.lines)) === Math.max(0, hunk.oldStart - 1),
  );
};
...
export const renderWorkspaceEdit = async (
  ...
  const rendered = await Promise.all(
    [...byUri.values()].map(async ({ uri, edits }) => {
