...
export function collectWorkspaceTextEdits(
  ...
  const textEditsFromChanges = Object.entries(edit.changes ?? {}).flatMap(
    ([uri, textEdits]) =>
  ...
  const textEditsFromDocumentChanges = (edit.documentChanges ?? []).flatMap((change) =>
  ...
  return [...textEditsFromChanges, ...textEditsFromDocumentChanges];
}
