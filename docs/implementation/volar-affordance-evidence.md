...
| Code-action discovery and resolution are separate Volar phases. | `@volar/language-service@2.4.28/lib/features/provideCodeActions.js`; `resolveCodeAction.js`; `@volar/language-server@2.4.28/lib/features/languageFeatures.js` | Listing preserves resolvable action data; resolution invokes the originating plugin and transforms the resolved edit and command. | `code_actions` lists the native actions and resolves only the displayed action selected by the caller. | Reuse |
| Code-action contexts accept multiple diagnostics and server-side kind filters. | `volar-service-typescript@0.0.65/lib/semanticFeatures/codeAction.js` | A single request iterates every diagnostic in `CodeActionContext.diagnostics`. `context.only` selects quick fixes, refactors, organize-imports, fix-all, remove-unused, and add-missing-import paths before TypeScript work is performed. | `code_actions` supplies the native diagnostics intersecting the requested range and forwards `only` as the protocol kind filter. It performs no action synthesis or kind filtering. | Reuse |
| TypeScript fixes, fix-all, organize imports, and refactors already become LSP edits. | `volar-service-typescript@0.0.65/lib/plugins/semantic.js`; `lib/semanticFeatures/codeAction.js`; `codeActionResolve.js`; `lib/utils/lspConverters.js` | Converts TypeScript `FileTextChanges` into `WorkspaceEdit` values, including create-file operations. Extract and rewrite refactors remain data-only when the client advertises resolution for both `edit` and `command`; resolution adds the edit and, when TypeScript returns a rename location, Volar's `editor.action.rename` command. | The MCP advertises both resolvable properties and never generates TypeScript fixes. Resolved edits enter the common patch renderer; editor-command actions are reported as unsupported rather than executed headlessly. | Reuse |
...
| LSP and the editor adapters do not render a headless folded document. | `vscode-languageserver-protocol@3.17.5/lib/common/protocol.foldingRange`; `@volar/monaco@2.4.28/lib/provider.ts` and `lib/languages.ts`; `monaco-languageserver-types@0.4.1` — `toFoldingRange`; `monaco-editor-core@0.55.1/esm/vs/editor/contrib/folding/browser/hiddenRangeModel.js` and `folding.js`; direct official-client MCP use | The Volar adapter forwards `getFoldingRanges` to Monaco's provider and converts zero-based LSP lines to one-based editor lines. Monaco's collapsed view hides `start + 1` through `end`, ignores ranges contained by an already hidden range, and calls `editor.setHiddenAreas`; the rest of its implementation manages editor decorations, cursor movement, persisted state, and commands. It exposes no headless text renderer. | `read_file` performs one physical read and one native folding request, then applies the same hidden-line boundary while retaining conventional one-based line numbers. Its presentation policy keeps views of 20 lines or fewer, upstream non-code ranges, and type declarations visible; requires a fold to hide at least six lines; and leaves any range spanning more than half the current view open so useful nested folds remain available. Inclusive line bounds enable whole-file outline, structurally folded range expansion, and exact bounded source through the same representation. It does not derive fold ranges or instantiate Monaco. | Required presentation boundary; verified over local stdio on small, type-centric, single-body, bounded, 850-line, and 2,269-line TypeScript views |
...
| A complete symbol working view is a client composition, not a separate semantic provider. | LSP hover, document-symbol, definition, type-definition, implementation, references, and call-hierarchy requests; `@volar/language-server@2.4.28` language-feature routing; `volar-service-typescript@0.0.71` semantic and syntactic plugins; direct official-client source-level MCP use on callable, interface, ambiguous-name, position-selected, source-expanded, bounded, and multi-workspace cases | The installed stack exposes each editor observation independently and has no `inspect` request. Document symbols select an exact file-local name and provide kind and declaration ranges; definition links provide the complete target body; call hierarchy supplies named callers, callees, and their bodies; references contain the remaining project-scoped locations. Volar deduplicates mapped results within a provider, but an exact duplicate call-site range can remain and the same declaration or call site legitimately appears across different providers. | `inspect_symbol` sends those native requests and adds only client concerns: exact-name ambiguity, cross-provider duplicate removal, file grouping, bounded sections, small physical source-line context for otherwise opaque references, and optional complete source. Callable type definitions and complete source remain off by default. The shared operation serves both MCP and native Codex tools; it performs no AST analysis, call discovery, type inference, source mapping, or project aggregation. | Required agent-presentation composition; directly verified |
...
| Organize imports is a resolved native code action. | `volar-service-typescript@0.0.71/lib/plugins/semantic.js` — `codeActionKinds`; `lib/semanticFeatures/codeAction.js`; `codeActionResolve.js`; `@volar/language-server@2.4.28/lib/features/languageFeatures.js` — code-action request and resolve handlers | Requesting only `source.organizeImports` returns one lightweight action; resolution calls TypeScript `organizeImports` and converts its `FileTextChanges` into an LSP `WorkspaceEdit`. | Do not create a bespoke organize-imports tool or implementation. The generic `code_actions` request/resolve path exposes the upstream action unchanged; agents may invoke the independent native formatting tool when the exact resolved edit requires formatting. | Reuse through generic code actions |
...
| Standard LSP code-action resolution drops TypeScript indentation options. | `@volar/language-server@2.4.28/lib/features/languageFeatures.js` — `onCodeActionResolve`; `volar-service-typescript@0.0.71/lib/plugins/semantic.js` and `lib/semanticFeatures/codeActionResolve.js`; direct source-level official-client use | The TypeScript plugin retains the most recent formatting options supplied to its formatting providers, but standard `codeAction/resolve` supplies only the action and cancellation token. A real missing-import quick fix inserted a multiline specifier at column zero; the independent native document-formatting request returned the exact one-line indentation repair. | Preserve both canonical operations rather than stitching a bespoke action pipeline: `code_actions` returns the resolved edit unchanged and `format_document` remains separately available. | Native limitation and composition directly observed |
...
| Physical source text remains client-owned at the LSP boundary. | LSP `WorkspaceEdit`; `@volar/language-server@2.4.28` server filesystem and document state; `@volar/kit@2.4.28` checker/formatter; `vscode-languageclient@9.0.1` apply-edit handler | A workspace edit carries locations and replacement text, not the original document. Volar's snapshots are process-internal; Kit obtains snapshots only by hosting the language service in-process, and the VS Code client delegates application to the editor-owned document. | The headless MCP client reads the current physical document and delegates range application to `vscode-languageserver-textdocument` before diff rendering. It does not add a private source-copy protocol. | Required client ownership |
